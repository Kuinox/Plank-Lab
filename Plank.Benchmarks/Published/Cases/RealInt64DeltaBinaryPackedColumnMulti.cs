...
[MemoryDiagnoser]
public class RealInt64DeltaBinaryPackedColumnMultiPlankBenchmarks
{
    ...
    public void CleanupWrite()
    {
        ...
        ColumnParallelism.WriteMarker("RealInt64DeltaBinaryPackedColumnMultiPlankBenchmarks", "write", _columnWorkerCount, _writeThreads);
        _output?.Dispose();
    }
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealInt64DeltaBinaryPackedColumnMultiPlankBenchmarks", "read", _columnWorkerCount, _readThreads);
...
[MemoryDiagnoser]
public class RealInt64DeltaBinaryPackedColumnMultiParquetSharpBenchmarks
{
    int _columnWorkerCount;
    readonly ColumnParallelism.Tracker _parallelism = new();
    int _readThreads;
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealInt64DeltaBinaryPackedColumnMultiParquetSharpBenchmarks", "read", _columnWorkerCount, _readThreads);
        foreach (var reader in _columnReaders ?? [])
            reader?.Dispose();
        foreach (var source in _columnSources ?? [])
            source?.Dispose();
