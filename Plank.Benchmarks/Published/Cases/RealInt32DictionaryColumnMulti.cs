...
[MemoryDiagnoser]
public class RealInt32DictionaryColumnMultiPlankBenchmarks
{
    ...
    public void GlobalSetupWrite()
    {
        ...
        _writer = RealInt32DictionaryRow.Schema.CreateWriter(preparationOutput, new ParquetWriterOptions
        ...
        _s0 = _writer.CreateSerializedColumn<int?>(RealInt32DictionaryRow.Schema.LeafColumns[0]);
        _s1 = _writer.CreateSerializedColumn<int?>(RealInt32DictionaryRow.Schema.LeafColumns[1]);
        _s2 = _writer.CreateSerializedColumn<int?>(RealInt32DictionaryRow.Schema.LeafColumns[2]);
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("RealInt32DictionaryColumn", "Plank", out _expectedOutputBytes)];
    ...
    public void CleanupWrite()
    {
        ...
        ColumnParallelism.WriteMarker("RealInt32DictionaryColumnMultiPlankBenchmarks", "write", _columnWorkerCount, _writeThreads);
        _output?.Dispose();
    }
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealInt32DictionaryColumnMultiPlankBenchmarks", "read", _columnWorkerCount, _readThreads);
...
[MemoryDiagnoser]
public class RealInt32DictionaryColumnMultiParquetSharpBenchmarks
{
    int _columnWorkerCount;
    readonly ColumnParallelism.Tracker _parallelism = new();
    int _readThreads;
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealInt32DictionaryColumnMultiParquetSharpBenchmarks", "read", _columnWorkerCount, _readThreads);
        foreach (var reader in _columnReaders ?? [])
            reader?.Dispose();
        foreach (var source in _columnSources ?? [])
            source?.Dispose();
