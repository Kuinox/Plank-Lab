...
[ParquetSchema]
sealed partial class RealTaxiDictionaryPlankRow
{
    ...
    [ParquetColumn("store_and_fwd_flag", Encodings = [EncodingKind.RleDictionary], LogicalType = LogicalTypeKind.String), MapToColumn("store_and_fwd_flag"), JsonPropertyName("store_and_fwd_flag")]
    ...
    public static byte[] CreateReadFile(RealTaxiDictionaryPlankRow[] rows)
    {
    ...
    public static RealTaxiDictionaryPlankRow[] FromSharp(RealTaxiDictionarySharpRow[] source)
    {
        var rows = new RealTaxiDictionaryPlankRow[source.Length];
        for (var index = 0; index < rows.Length; index++)
        {
            var value = source[index];
            rows[index] = new RealTaxiDictionaryPlankRow
            {
    ...
    public static RealTaxiDictionaryPlankRow[] FromNet(RealTaxiDictionaryNetRow[] source)
    {
        var rows = new RealTaxiDictionaryPlankRow[source.Length];
        for (var index = 0; index < rows.Length; index++)
        {
...
sealed partial class RealTaxiDictionarySharpRow
{
...
sealed partial class RealTaxiDictionaryNetRow
{
    [MapToColumn("VendorID"), JsonPropertyName("VendorID")]
    public int? VendorId { get; set; }
...
[MemoryDiagnoser]
public class RealTaxiDictionaryPlankBenchmarks
{
    ...
    RealTaxiDictionaryPlankRow.RowReader _reader = null!;
    ...
    public void GlobalSetupWrite()
    {
        ...
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("RealTaxiDictionary", "Plank", out _expectedOutputBytes)];
    }
    ...
    public void GlobalSetupRead()
    {
        ...
        _reader = RealTaxiDictionaryPlankRow.CreateRowReader(_source, options: new RowReaderOptions { BufferPool = _pool });
    }
...
[MemoryDiagnoser]
public class RealTaxiDictionaryParquetSharpBenchmarks
{
    ...
    ParquetRowWriter<RealTaxiDictionarySharpRow> _writer = null!;
    byte[] _outputBuffer = null!;
    long _expectedOutputBytes;
    GCHandle _pinned;
    NativeBuffer _buffer = null!;
    BufferReader _source = null!;
    ParquetRowReader<RealTaxiDictionarySharpRow>? _reader;
    ...
    public void GlobalSetupWrite()
    {
        ...
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("RealTaxiDictionary", "ParquetSharp", out _expectedOutputBytes)];
    }
    ...
    public void SetupWrite()
    {
        ...
        _writer = ParquetFile.CreateRowWriter<RealTaxiDictionarySharpRow>(_managedOutput, _properties, _schema);
    }
    ...
    {
        _reader?.Dispose();
        _reader = ParquetFile.CreateRowReader<RealTaxiDictionarySharpRow>(_source);
    }
...
[MemoryDiagnoser]
public class RealTaxiDictionaryParquetNetBenchmarks
{
    byte[] _file = null!;
    ...
    [GlobalSetup(Target = nameof(Read))]
    public void GlobalSetupRead() => _file = BenchmarkFixtures.LoadReadFile("RealTaxiDictionary");
    ...
    {
        var result = await ParquetSerializer.DeserializeAsync<RealTaxiDictionaryNetRow>(_stream!);
        double sum = 0;
