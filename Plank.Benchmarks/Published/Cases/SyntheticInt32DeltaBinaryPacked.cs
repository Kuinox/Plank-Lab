...
[ParquetSchema]
sealed partial class SyntheticInt32DeltaBinaryPackedRow
{
    ...
    public static SyntheticInt32DeltaBinaryPackedRow[] CreateRows(int count)
    {
        var rows = new SyntheticInt32DeltaBinaryPackedRow[count];
        for (var row = 0; row < rows.Length; row++)
            rows[row] = new SyntheticInt32DeltaBinaryPackedRow
            {
    ...
    public static byte[] CreateReadFile(SyntheticInt32DeltaBinaryPackedRow[] rows)
    {
...
[MemoryDiagnoser]
public class SyntheticInt32DeltaBinaryPackedPlankBenchmarks
{
    SyntheticInt32DeltaBinaryPackedRow[] _rows = null!;
    DefaultParquetBufferPool _pool = null!;
    ParquetWriterOptions _options = null!;
    SyntheticInt32DeltaBinaryPackedRow.PipelineWriter _writer = null!;
    MemoryStream _output = null!;
    byte[] _outputBuffer = null!;
    long _expectedOutputBytes;
    MemoryReadSource _source = null!;
    SyntheticInt32DeltaBinaryPackedRow.RowReader _reader = null!;
    ...
    {
        _rows ??= SyntheticInt32DeltaBinaryPackedRow.CreateRows(Rows);
        ...
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("SyntheticInt32DeltaBinaryPacked", "Plank", out _expectedOutputBytes)];
    }
    ...
    {
        _pool = new DefaultParquetBufferPool(ParquetBufferRetentionPolicy.ZeroAllocation);
        var file = BenchmarkFixtures.LoadReadFile("SyntheticInt32DeltaBinaryPacked");
        _source = new MemoryReadSource(file);
        _reader = SyntheticInt32DeltaBinaryPackedRow.CreateRowReader(_source, options: new RowReaderOptions { BufferPool = _pool });
    }
...
[MemoryDiagnoser]
public class SyntheticInt32DeltaBinaryPackedParquetSharpBenchmarks
{
    ...
    ParquetRowWriter<SyntheticInt32DeltaBinaryPackedRow> _writer = null!;
    byte[] _outputBuffer = null!;
    long _expectedOutputBytes;
    GCHandle _pinned;
    NativeBuffer _buffer = null!;
    BufferReader _source = null!;
    ParquetRowReader<SyntheticInt32DeltaBinaryPackedRow>? _reader;
    ...
    public void GlobalSetupWrite()
    {
        ...
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("SyntheticInt32DeltaBinaryPacked", "ParquetSharp", out _expectedOutputBytes)];
    }
    ...
    public void SetupWrite()
    {
        ...
        _writer = ParquetFile.CreateRowWriter<SyntheticInt32DeltaBinaryPackedRow>(_managedOutput, _properties, _schema);
    }
    ...
    {
        _reader?.Dispose();
        _reader = ParquetFile.CreateRowReader<SyntheticInt32DeltaBinaryPackedRow>(_source);
    }
...
[MemoryDiagnoser]
public class SyntheticInt32DeltaBinaryPackedParquetNetBenchmarks
{
    SyntheticInt32DeltaBinaryPackedRow[] _rows = null!;
    ...
    public void GlobalSetupWrite()
    {
        ...
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("SyntheticInt32DeltaBinaryPacked", "Parquet.Net", out _expectedOutputBytes)];
    }

    [GlobalSetup(Target = nameof(Read))]
    public void GlobalSetupRead() => _file = BenchmarkFixtures.LoadReadFile("SyntheticInt32DeltaBinaryPacked");
    ...
    {
        var result = await ParquetSerializer.DeserializeAsync<SyntheticInt32DeltaBinaryPackedRow>(_stream!);
        ulong sum = 0;
