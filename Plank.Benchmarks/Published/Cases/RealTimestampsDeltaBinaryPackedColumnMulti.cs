...
[MemoryDiagnoser]
public class RealTimestampsDeltaBinaryPackedColumnMultiPlankBenchmarks
{
    ...
    public void GlobalSetupWrite()
    {
        ...
        _writer = RealTimestampsDeltaBinaryPackedRow.Schema.CreateWriter(preparationOutput, new ParquetWriterOptions
        ...
        _s0 = _writer.CreateSerializedColumn<DateTime?>(RealTimestampsDeltaBinaryPackedRow.Schema.LeafColumns[0]);
        _s1 = _writer.CreateSerializedColumn<DateTime?>(RealTimestampsDeltaBinaryPackedRow.Schema.LeafColumns[1]);
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("RealTimestampsDeltaBinaryPackedColumn", "Plank", out _expectedOutputBytes)];
    ...
    public void CleanupWrite()
    {
        ...
        ColumnParallelism.WriteMarker("RealTimestampsDeltaBinaryPackedColumnMultiPlankBenchmarks", "write", _columnWorkerCount, _writeThreads);
        _output?.Dispose();
    }
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealTimestampsDeltaBinaryPackedColumnMultiPlankBenchmarks", "read", _columnWorkerCount, _readThreads);
...
[MemoryDiagnoser]
public class RealTimestampsDeltaBinaryPackedColumnMultiParquetSharpBenchmarks
{
    int _columnWorkerCount;
    readonly ColumnParallelism.Tracker _parallelism = new();
    int _readThreads;
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealTimestampsDeltaBinaryPackedColumnMultiParquetSharpBenchmarks", "read", _columnWorkerCount, _readThreads);
        foreach (var reader in _columnReaders ?? [])
            reader?.Dispose();
        foreach (var source in _columnSources ?? [])
            source?.Dispose();
