...
[ParquetSchema]
sealed partial class SyntheticInt32DictionaryRow
{
    ...
    public static SyntheticInt32DictionaryRow[] CreateRows(int count)
    {
        var rows = new SyntheticInt32DictionaryRow[count];
        for (var row = 0; row < rows.Length; row++)
            rows[row] = new SyntheticInt32DictionaryRow
            {
    ...
    public static byte[] CreateReadFile(SyntheticInt32DictionaryRow[] rows)
    {
...
[MemoryDiagnoser]
public class SyntheticInt32DictionaryPlankBenchmarks
{
    SyntheticInt32DictionaryRow[] _rows = null!;
    ...
    public void Setup()
    {
        _rows = SyntheticInt32DictionaryRow.CreateRows(Rows);
        ...
        Console.WriteLine("BENCHMARK_FILE|SyntheticInt32Dictionary|Plank|" + file.Length);
        _outputCapacity = BenchmarkData.OutputCapacity(file.Length);
        _output.Dispose();

        _source = new MemoryReadSource(file);
        _reader = SyntheticInt32DictionaryRow.CreateRowReader(_source, options: new RowReaderOptions { BufferPool = _pool });
        _ = Read();
    }
    ...
    public void Write()
    {
        using var writer = SyntheticInt32DictionaryRow.CreateRowWriter(_output, _options);
        _pinning.Wait();
...
[MemoryDiagnoser]
public class SyntheticInt32DictionaryParquetSharpBenchmarks
{
    ...
    ParquetRowReader<SyntheticInt32DictionaryRow>? _reader;
    ...
    public void Setup()
    {
        ...
        Console.WriteLine("BENCHMARK_FILE|SyntheticInt32Dictionary|ParquetSharp|" + _output.Length);
        _writer.Dispose();
        _managedOutput.Dispose();
        _output.Dispose();

        var file = SyntheticInt32DictionaryRow.CreateReadFile(_rows);
    ...
    {
        _reader?.Dispose();
        _reader = ParquetFile.CreateRowReader<SyntheticInt32DictionaryRow>(_source);
    }
...
[MemoryDiagnoser]
public class SyntheticInt32DictionaryParquetNetBenchmarks
{
    SyntheticInt32DictionaryRow[] _rows = null!;
    byte[] _file = null!;
    ...
    public void Setup()
    {
        ...
        _file = SyntheticInt32DictionaryRow.CreateReadFile(_rows);
    }
    ...
    {
        var result = await ParquetSerializer.DeserializeAsync<SyntheticInt32DictionaryRow>(_stream!);
        ulong sum = 0;
