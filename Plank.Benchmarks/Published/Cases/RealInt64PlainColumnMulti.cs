...
[MemoryDiagnoser]
public class RealInt64PlainColumnMultiPlankBenchmarks
{
    ...
    public void CleanupWrite()
    {
        ...
        ColumnParallelism.WriteMarker("RealInt64PlainColumnMultiPlankBenchmarks", "write", _columnWorkerCount, _writeThreads);
        _output?.Dispose();
    }
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealInt64PlainColumnMultiPlankBenchmarks", "read", _columnWorkerCount, _readThreads);
...
[MemoryDiagnoser]
public class RealInt64PlainColumnMultiParquetSharpBenchmarks
{
    int _columnWorkerCount;
    readonly ColumnParallelism.Tracker _parallelism = new();
    int _readThreads;
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealInt64PlainColumnMultiParquetSharpBenchmarks", "read", _columnWorkerCount, _readThreads);
        foreach (var reader in _columnReaders ?? [])
            reader?.Dispose();
        foreach (var source in _columnSources ?? [])
            source?.Dispose();
