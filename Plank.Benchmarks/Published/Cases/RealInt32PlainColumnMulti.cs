...
[MemoryDiagnoser]
public class RealInt32PlainColumnMultiPlankBenchmarks
{
    ...
    public void CleanupWrite()
    {
        ...
        ColumnParallelism.WriteMarker("RealInt32PlainColumnMultiPlankBenchmarks", "write", _columnWorkerCount, _writeThreads);
        _output?.Dispose();
    }
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealInt32PlainColumnMultiPlankBenchmarks", "read", _columnWorkerCount, _readThreads);
...
[MemoryDiagnoser]
public class RealInt32PlainColumnMultiParquetSharpBenchmarks
{
    int _columnWorkerCount;
    readonly ColumnParallelism.Tracker _parallelism = new();
    int _readThreads;
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealInt32PlainColumnMultiParquetSharpBenchmarks", "read", _columnWorkerCount, _readThreads);
        foreach (var reader in _columnReaders ?? [])
            reader?.Dispose();
        foreach (var source in _columnSources ?? [])
            source?.Dispose();
