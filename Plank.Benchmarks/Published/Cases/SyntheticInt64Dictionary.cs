...
[ParquetSchema]
sealed partial class SyntheticInt64DictionaryRow
{
    ...
    public static SyntheticInt64DictionaryRow[] CreateRows(int count)
    {
        var rows = new SyntheticInt64DictionaryRow[count];
        for (var row = 0; row < rows.Length; row++)
            rows[row] = new SyntheticInt64DictionaryRow
            {
    ...
    public static byte[] CreateReadFile(SyntheticInt64DictionaryRow[] rows)
    {
...
[MemoryDiagnoser]
public class SyntheticInt64DictionaryPlankBenchmarks
{
    ...
    SyntheticInt64DictionaryRow.RowReader _reader = null!;
    ...
    {
        _rows ??= SyntheticInt64DictionaryRow.CreateRows(Rows);
        ...
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("SyntheticInt64Dictionary", "Plank", out _expectedOutputBytes)];
    }
    ...
    {
        _pool = new DefaultParquetBufferPool(ParquetBufferRetentionPolicy.ZeroAllocation);
        var file = BenchmarkFixtures.LoadReadFile("SyntheticInt64Dictionary");
        _source = new MemoryReadSource(file);
        _reader = SyntheticInt64DictionaryRow.CreateRowReader(_source, options: new RowReaderOptions { BufferPool = _pool });
    }
...
[MemoryDiagnoser]
public class SyntheticInt64DictionaryParquetSharpBenchmarks
{
    ...
    ParquetRowWriter<SyntheticInt64DictionaryRow> _writer = null!;
    byte[] _outputBuffer = null!;
    long _expectedOutputBytes;
    GCHandle _pinned;
    NativeBuffer _buffer = null!;
    BufferReader _source = null!;
    ParquetRowReader<SyntheticInt64DictionaryRow>? _reader;
    ...
    public void GlobalSetupWrite()
    {
        ...
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("SyntheticInt64Dictionary", "ParquetSharp", out _expectedOutputBytes)];
    }
    ...
    public void SetupWrite()
    {
        ...
        _writer = ParquetFile.CreateRowWriter<SyntheticInt64DictionaryRow>(_managedOutput, _properties, _schema);
    }
    ...
    {
        _reader?.Dispose();
        _reader = ParquetFile.CreateRowReader<SyntheticInt64DictionaryRow>(_source);
    }
...
[MemoryDiagnoser]
public class SyntheticInt64DictionaryParquetNetBenchmarks
{
    byte[] _file = null!;
    ...
    [GlobalSetup(Target = nameof(Read))]
    public void GlobalSetupRead() => _file = BenchmarkFixtures.LoadReadFile("SyntheticInt64Dictionary");
    ...
    {
        var result = await ParquetSerializer.DeserializeAsync<SyntheticInt64DictionaryRow>(_stream!);
        ulong sum = 0;
