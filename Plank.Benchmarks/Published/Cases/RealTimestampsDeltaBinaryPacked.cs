...
[ParquetSchema]
sealed partial class RealTimestampsDeltaBinaryPackedRow
{
    [ParquetColumn("tpep_pickup_datetime", Encodings = [EncodingKind.DeltaBinaryPacked], LogicalType = LogicalTypeKind.Timestamp), MapToColumn("tpep_pickup_datetime"), JsonPropertyName("tpep_pickup_datetime")]
    public DateTime? Pickup { get; set; }

    [ParquetColumn("tpep_dropoff_datetime", Encodings = [EncodingKind.DeltaBinaryPacked], LogicalType = LogicalTypeKind.Timestamp), MapToColumn("tpep_dropoff_datetime"), JsonPropertyName("tpep_dropoff_datetime")]
    public DateTime? Dropoff { get; set; }

    public static byte[] CreateReadFile(RealTimestampsDeltaBinaryPackedRow[] rows)
    {
...
[MemoryDiagnoser]
public class RealTimestampsDeltaBinaryPackedPlankBenchmarks
{
    ...
    RealTimestampsDeltaBinaryPackedRow.RowReader _reader = null!;
    ...
    {
        _rows ??= BenchmarkData.LoadTaxiRows<RealTimestampsDeltaBinaryPackedRow>();
        ...
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("RealTimestampsDeltaBinaryPacked", "Plank", out _expectedOutputBytes)];
    }
    ...
    {
        _pool = new DefaultParquetBufferPool(ParquetBufferRetentionPolicy.ZeroAllocation);
        var file = BenchmarkFixtures.LoadReadFile("RealTimestampsDeltaBinaryPacked");
        _source = new MemoryReadSource(file);
        _reader = RealTimestampsDeltaBinaryPackedRow.CreateRowReader(_source, options: new RowReaderOptions { BufferPool = _pool });
    }
...
[MemoryDiagnoser]
public class RealTimestampsDeltaBinaryPackedParquetSharpBenchmarks
{
    ...
    ParquetRowWriter<RealTimestampsDeltaBinaryPackedRow> _writer = null!;
    byte[] _outputBuffer = null!;
    long _expectedOutputBytes;
    GCHandle _pinned;
    NativeBuffer _buffer = null!;
    BufferReader _source = null!;
    ParquetRowReader<RealTimestampsDeltaBinaryPackedRow>? _reader;
    ...
    public void GlobalSetupWrite()
    {
        ...
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("RealTimestampsDeltaBinaryPacked", "ParquetSharp", out _expectedOutputBytes)];
    }
    ...
    public void SetupWrite()
    {
        ...
        _writer = ParquetFile.CreateRowWriter<RealTimestampsDeltaBinaryPackedRow>(_managedOutput, _properties, _schema);
    }
    ...
    {
        _reader?.Dispose();
        _reader = ParquetFile.CreateRowReader<RealTimestampsDeltaBinaryPackedRow>(_source);
    }
