...
[MemoryDiagnoser]
public class RealTimestampsDictionaryColumnMultiPlankBenchmarks
{
    ...
    public void GlobalSetupWrite()
    {
        ...
        _writer = RealTimestampsDictionaryRow.Schema.CreateWriter(preparationOutput, new ParquetWriterOptions
        ...
        _s0 = _writer.CreateSerializedColumn<DateTime?>(RealTimestampsDictionaryRow.Schema.LeafColumns[0]);
        _s1 = _writer.CreateSerializedColumn<DateTime?>(RealTimestampsDictionaryRow.Schema.LeafColumns[1]);
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("RealTimestampsDictionaryColumn", "Plank", out _expectedOutputBytes)];
    ...
    public void CleanupWrite()
    {
        ...
        ColumnParallelism.WriteMarker("RealTimestampsDictionaryColumnMultiPlankBenchmarks", "write", _columnWorkerCount, _writeThreads);
        _output?.Dispose();
    }
...
[MemoryDiagnoser]
public class RealTimestampsDictionaryColumnMultiParquetSharpBenchmarks
{
    int _columnWorkerCount;
    readonly ColumnParallelism.Tracker _parallelism = new();
    int _readThreads;
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealTimestampsDictionaryColumnMultiParquetSharpBenchmarks", "read", _columnWorkerCount, _readThreads);
        foreach (var reader in _columnReaders ?? [])
            reader?.Dispose();
        foreach (var source in _columnSources ?? [])
            source?.Dispose();
