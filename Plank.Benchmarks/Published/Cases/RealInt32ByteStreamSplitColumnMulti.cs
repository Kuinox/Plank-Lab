...
public class RealInt32ByteStreamSplitColumnMultiPlankBenchmarks
{
    ...
    public void CleanupWrite()
    {
        ...
        ColumnParallelism.WriteMarker("RealInt32ByteStreamSplitColumnMultiPlankBenchmarks", "write", _columnWorkerCount, _writeThreads);
        _output?.Dispose();
    }
...
public class RealInt32ByteStreamSplitColumnMultiParquetSharpBenchmarks
{
    ...
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealInt32ByteStreamSplitColumnMultiParquetSharpBenchmarks", "read", _columnWorkerCount, _readThreads);
        foreach (var reader in _columnReaders ?? [])
            reader?.Dispose();
        foreach (var source in _columnSources ?? [])
            source?.Dispose();
