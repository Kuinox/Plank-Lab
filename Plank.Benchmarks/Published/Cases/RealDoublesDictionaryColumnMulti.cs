using System.Runtime.InteropServices;
using System.Text;
using System.Text.Json.Serialization;
using BenchmarkDotNet.Attributes;
using Parquet;
using Parquet.Serialization;
using ParquetSharp;
using ParquetSharp.IO;
using ParquetSharp.RowOriented;
using Plank.Reading;
using Plank.RowApi;
using Plank.Schema;
using Plank.Writing;
using NativeBuffer = ParquetSharp.IO.Buffer;
using Encoding = System.Text.Encoding;
using LogicalType = ParquetSharp.LogicalType;
using TimeUnit = ParquetSharp.TimeUnit;
using ParquetDataPageVersion = Plank.Writing.ParquetDataPageVersion;

namespace Plank.Benchmarks;

// Generated by scripts/generate_column_cases.py. Row cases remain unchanged.
// Generated by scripts/generate_column_cases.py. Multi adapters own one reader or serializer per column.
[MemoryDiagnoser]
public class RealDoublesDictionaryColumnMultiPlankBenchmarks
{
    int _columnWorkerCount;
    readonly ColumnParallelism.Tracker _parallelism = new();
    int _writeThreads;
    int _readThreads;
    MemoryReadSource[] _columnSources = null!;
    Plank.Reading.Logical.ParquetReader[] _columnReaders = null!;
    long[] _columnCounts = null!;

    const int RowsPerRowGroup = 1048576;
    RealDoublesDictionaryRow[] _rows = null!;
    MemoryStream _output = null!;
    byte[] _outputBuffer = null!;
    long _expectedOutputBytes;
    double?[] _c0 = null!;
    double?[] _c1 = null!;
    double?[] _c2 = null!;
    double?[] _c3 = null!;
    double?[] _c4 = null!;
    double?[] _c5 = null!;
    double?[] _c6 = null!;
    double?[] _c7 = null!;
    double?[] _c8 = null!;
    double?[] _c9 = null!;
    DefaultParquetBufferPool _pool = null!;
    Plank.Writing.ParquetWriter _writer = null!;
    MemoryReadSource _source = null!;
    Plank.Reading.Logical.ParquetReader _reader = null!;
    SerializedColumn<double?> _s0 = null!;
    SerializedColumn<double?> _s1 = null!;
    SerializedColumn<double?> _s2 = null!;
    SerializedColumn<double?> _s3 = null!;
    SerializedColumn<double?> _s4 = null!;
    SerializedColumn<double?> _s5 = null!;
    SerializedColumn<double?> _s6 = null!;
    SerializedColumn<double?> _s7 = null!;
    SerializedColumn<double?> _s8 = null!;
    SerializedColumn<double?> _s9 = null!;
    public IEnumerable<int> RowCounts => [BenchmarkData.TaxiRows];
    [ParamsSource(nameof(RowCounts))]
    public int Rows { get; set; }
    [GlobalSetup(Target = nameof(Write))]
    public void GlobalSetupWrite()
    {


        _rows ??= BenchmarkData.LoadTaxiRows<RealDoublesDictionaryRow>();
        _c0 = _rows.Select(value => value.TripDistance).ToArray();
        _c1 = _rows.Select(value => value.FareAmount).ToArray();
        _c2 = _rows.Select(value => value.Extra).ToArray();
        _c3 = _rows.Select(value => value.MtaTax).ToArray();
        _c4 = _rows.Select(value => value.TipAmount).ToArray();
        _c5 = _rows.Select(value => value.TollsAmount).ToArray();
        _c6 = _rows.Select(value => value.ImprovementSurcharge).ToArray();
        _c7 = _rows.Select(value => value.TotalAmount).ToArray();
        _c8 = _rows.Select(value => value.CongestionSurcharge).ToArray();
        _c9 = _rows.Select(value => value.AirportFee).ToArray();
        _rows = null!;
        _pool = new DefaultParquetBufferPool(ParquetBufferRetentionPolicy.ZeroAllocation);
        using var preparationOutput = new MemoryStream();
        _writer = RealDoublesDictionaryRow.Schema.CreateWriter(preparationOutput, new ParquetWriterOptions
        { Compression = CompressionKind.None, BufferPool = _pool, DataPageVersion = ParquetDataPageVersion.V2,
           WritePageIndexes = true, WritePageCrc = false });
        _s0 = _writer.CreateSerializedColumn<double?>(RealDoublesDictionaryRow.Schema.LeafColumns[0]);
        _s1 = _writer.CreateSerializedColumn<double?>(RealDoublesDictionaryRow.Schema.LeafColumns[1]);
        _s2 = _writer.CreateSerializedColumn<double?>(RealDoublesDictionaryRow.Schema.LeafColumns[2]);
        _s3 = _writer.CreateSerializedColumn<double?>(RealDoublesDictionaryRow.Schema.LeafColumns[3]);
        _s4 = _writer.CreateSerializedColumn<double?>(RealDoublesDictionaryRow.Schema.LeafColumns[4]);
        _s5 = _writer.CreateSerializedColumn<double?>(RealDoublesDictionaryRow.Schema.LeafColumns[5]);
        _s6 = _writer.CreateSerializedColumn<double?>(RealDoublesDictionaryRow.Schema.LeafColumns[6]);
        _s7 = _writer.CreateSerializedColumn<double?>(RealDoublesDictionaryRow.Schema.LeafColumns[7]);
        _s8 = _writer.CreateSerializedColumn<double?>(RealDoublesDictionaryRow.Schema.LeafColumns[8]);
        _s9 = _writer.CreateSerializedColumn<double?>(RealDoublesDictionaryRow.Schema.LeafColumns[9]);
        _outputBuffer = new byte[BenchmarkFixtures.GetOutputCapacity("RealDoublesDictionaryColumn", "Plank", out _expectedOutputBytes)];
        _columnWorkerCount = ColumnParallelism.WorkerCount(10);
    }
    [IterationSetup(Target = nameof(Write))]
    public void SetupWrite()
    {
        _output = BenchmarkFixtures.CreateOutput(_outputBuffer);
        _writer.Reset(_output);
    }
    void SerializeColumn(int ordinal, int start, int count)
    {
        switch (ordinal)
        {
            case 0: _s0.Serialize(_c0.AsSpan(start, count)); break;
            case 1: _s1.Serialize(_c1.AsSpan(start, count)); break;
            case 2: _s2.Serialize(_c2.AsSpan(start, count)); break;
            case 3: _s3.Serialize(_c3.AsSpan(start, count)); break;
            case 4: _s4.Serialize(_c4.AsSpan(start, count)); break;
            case 5: _s5.Serialize(_c5.AsSpan(start, count)); break;
            case 6: _s6.Serialize(_c6.AsSpan(start, count)); break;
            case 7: _s7.Serialize(_c7.AsSpan(start, count)); break;
            case 8: _s8.Serialize(_c8.AsSpan(start, count)); break;
            case 9: _s9.Serialize(_c9.AsSpan(start, count)); break;
            default: throw new ArgumentOutOfRangeException(nameof(ordinal));
        }
    }
    [Benchmark]
    public void Write()
    {
        for (var start = 0; start < Rows; start += RowsPerRowGroup)
        {
            var count = Math.Min(RowsPerRowGroup, Rows - start);
            _writeThreads = Math.Max(_writeThreads, _parallelism.Run(10, _columnWorkerCount,
                index => SerializeColumn(index, start, count)));
            var group = _writer.StartRowGroup();
            group.Write(_s0);
            group.Write(_s1);
            group.Write(_s2);
            group.Write(_s3);
            group.Write(_s4);
            group.Write(_s5);
            group.Write(_s6);
            group.Write(_s7);
            group.Write(_s8);
            group.Write(_s9);
        }
        _writer.CloseFile();
    }
    [IterationCleanup(Target = nameof(Write))]
    public void CleanupWrite()
    {
        BenchmarkFixtures.ValidateOutput(_expectedOutputBytes, BenchmarkFixtures.OutputLength(_output));
        ColumnParallelism.WriteMarker("RealDoublesDictionaryColumnMultiPlankBenchmarks", "write", _columnWorkerCount, _writeThreads);
        _output?.Dispose();
    }
    [GlobalSetup(Target = nameof(Read))]
    public void GlobalSetupRead()
    {
        _pool = new DefaultParquetBufferPool(ParquetBufferRetentionPolicy.ZeroAllocation);
        var file = BenchmarkFixtures.LoadReadFile("RealDoublesDictionary");
        _columnSources = new MemoryReadSource[10];
        _columnReaders = new Plank.Reading.Logical.ParquetReader[10];
        _columnCounts = new long[10];
        for (var index = 0; index < 10; index++)
        {
            _columnSources[index] = new MemoryReadSource(file);
            _columnReaders[index] = new Plank.Reading.Logical.ParquetReader(
                new Plank.Reading.Logical.ParquetReaderOptions { BufferPool = _pool });
        }
        _source = _columnSources[0];
        _reader = _columnReaders[0];
        _columnWorkerCount = ColumnParallelism.WorkerCount(10);
    }
    [IterationSetup(Target = nameof(Read))]
    public void SetupRead()
    {
        for (var index = 0; index < 10; index++)
            _columnReaders[index].Reset(_columnSources[index]);
    }
    public double VerifyRead()
    {
        double sum = 0;
        long count = 0;
        foreach (var group in _reader.RowGroups)
        {
            foreach (var buffer in group.Column<double?>(0))
            {
                sum = ColumnChecksum.Accumulate(buffer.Values, sum);
                count += buffer.Count;
            }
            foreach (var buffer in group.Column<double?>(1))
            {
                sum = ColumnChecksum.Accumulate(buffer.Values, sum);
                count += buffer.Count;
            }
            foreach (var buffer in group.Column<double?>(2))
            {
                sum = ColumnChecksum.Accumulate(buffer.Values, sum);
                count += buffer.Count;
            }
            foreach (var buffer in group.Column<double?>(3))
            {
                sum = ColumnChecksum.Accumulate(buffer.Values, sum);
                count += buffer.Count;
            }
            foreach (var buffer in group.Column<double?>(4))
            {
                sum = ColumnChecksum.Accumulate(buffer.Values, sum);
                count += buffer.Count;
            }
            foreach (var buffer in group.Column<double?>(5))
            {
                sum = ColumnChecksum.Accumulate(buffer.Values, sum);
                count += buffer.Count;
            }
            foreach (var buffer in group.Column<double?>(6))
            {
                sum = ColumnChecksum.Accumulate(buffer.Values, sum);
                count += buffer.Count;
            }
            foreach (var buffer in group.Column<double?>(7))
            {
                sum = ColumnChecksum.Accumulate(buffer.Values, sum);
                count += buffer.Count;
            }
            foreach (var buffer in group.Column<double?>(8))
            {
                sum = ColumnChecksum.Accumulate(buffer.Values, sum);
                count += buffer.Count;
            }
            foreach (var buffer in group.Column<double?>(9))
            {
                sum = ColumnChecksum.Accumulate(buffer.Values, sum);
                count += buffer.Count;
            }
        }
        if (count != (long)Rows * 10) throw new InvalidDataException($"Expected {(long)Rows * 10} values, got {count}.");
        return sum;
    }

    long ReadColumn0()
    {
        long count = 0;
        foreach (var group in _columnReaders[0].RowGroups)
        {
            foreach (var buffer in group.Column<double?>(0))
            {
                ReadConsumption.Consume(buffer.Values);
                count += buffer.Count;
            }
        }
        return count;
    }

    long ReadColumn1()
    {
        long count = 0;
        foreach (var group in _columnReaders[1].RowGroups)
        {
            foreach (var buffer in group.Column<double?>(1))
            {
                ReadConsumption.Consume(buffer.Values);
                count += buffer.Count;
            }
        }
        return count;
    }

    long ReadColumn2()
    {
        long count = 0;
        foreach (var group in _columnReaders[2].RowGroups)
        {
            foreach (var buffer in group.Column<double?>(2))
            {
                ReadConsumption.Consume(buffer.Values);
                count += buffer.Count;
            }
        }
        return count;
    }

    long ReadColumn3()
    {
        long count = 0;
        foreach (var group in _columnReaders[3].RowGroups)
        {
            foreach (var buffer in group.Column<double?>(3))
            {
                ReadConsumption.Consume(buffer.Values);
                count += buffer.Count;
            }
        }
        return count;
    }

    long ReadColumn4()
    {
        long count = 0;
        foreach (var group in _columnReaders[4].RowGroups)
        {
            foreach (var buffer in group.Column<double?>(4))
            {
                ReadConsumption.Consume(buffer.Values);
                count += buffer.Count;
            }
        }
        return count;
    }

    long ReadColumn5()
    {
        long count = 0;
        foreach (var group in _columnReaders[5].RowGroups)
        {
            foreach (var buffer in group.Column<double?>(5))
            {
                ReadConsumption.Consume(buffer.Values);
                count += buffer.Count;
            }
        }
        return count;
    }

    long ReadColumn6()
    {
        long count = 0;
        foreach (var group in _columnReaders[6].RowGroups)
        {
            foreach (var buffer in group.Column<double?>(6))
            {
                ReadConsumption.Consume(buffer.Values);
                count += buffer.Count;
            }
        }
        return count;
    }

    long ReadColumn7()
    {
        long count = 0;
        foreach (var group in _columnReaders[7].RowGroups)
        {
            foreach (var buffer in group.Column<double?>(7))
            {
                ReadConsumption.Consume(buffer.Values);
                count += buffer.Count;
            }
        }
        return count;
    }

    long ReadColumn8()
    {
        long count = 0;
        foreach (var group in _columnReaders[8].RowGroups)
        {
            foreach (var buffer in group.Column<double?>(8))
            {
                ReadConsumption.Consume(buffer.Values);
                count += buffer.Count;
            }
        }
        return count;
    }

    long ReadColumn9()
    {
        long count = 0;
        foreach (var group in _columnReaders[9].RowGroups)
        {
            foreach (var buffer in group.Column<double?>(9))
            {
                ReadConsumption.Consume(buffer.Values);
                count += buffer.Count;
            }
        }
        return count;
    }

    long ReadColumn(int ordinal)
        => ordinal switch
        {
            0 => ReadColumn0(),
            1 => ReadColumn1(),
            2 => ReadColumn2(),
            3 => ReadColumn3(),
            4 => ReadColumn4(),
            5 => ReadColumn5(),
            6 => ReadColumn6(),
            7 => ReadColumn7(),
            8 => ReadColumn8(),
            9 => ReadColumn9(),
            _ => throw new ArgumentOutOfRangeException(nameof(ordinal))
        };

    [Benchmark]
    public long Read()
    {
        _parallelism.Run(10, _columnWorkerCount,
            index => _columnCounts[index] = ReadColumn(index));
        long count = 0;
        for (var index = 0; index < 10; index++)
            count += _columnCounts[index];
        if (count != (long)Rows * 10)
            throw new InvalidDataException($"Expected {(long)Rows * 10} values, got {count}.");
        _readThreads = _parallelism.LastObserved;
        return count;
    }

    [GlobalCleanup]
    public void Cleanup()
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealDoublesDictionaryColumnMultiPlankBenchmarks", "read", _columnWorkerCount, _readThreads);
        foreach (var reader in _columnReaders ?? [])
            reader?.Dispose();
        foreach (var source in _columnSources ?? [])
            source?.Dispose();
        _writer?.Dispose();
        _pool?.Dispose();
        _output?.Dispose();
    }
}
[MemoryDiagnoser]
public class RealDoublesDictionaryColumnMultiParquetSharpBenchmarks
{
    int _columnWorkerCount;
    readonly ColumnParallelism.Tracker _parallelism = new();
    int _readThreads;
    ParquetFileReader[] _columnReaders = null!;
    BufferReader[] _columnSources = null!;
    long[] _columnCounts = null!;

    const int RowsPerRowGroup = 1048576;
    ParquetFileReader _reader = null!;
    GCHandle _pinned;
    NativeBuffer _buffer = null!;
    BufferReader _source = null!;
    double?[] _read0 = new double?[65536];
    double?[] _read1 = new double?[65536];
    double?[] _read2 = new double?[65536];
    double?[] _read3 = new double?[65536];
    double?[] _read4 = new double?[65536];
    double?[] _read5 = new double?[65536];
    double?[] _read6 = new double?[65536];
    double?[] _read7 = new double?[65536];
    double?[] _read8 = new double?[65536];
    double?[] _read9 = new double?[65536];
    public IEnumerable<int> RowCounts => [BenchmarkData.TaxiRows];
    [ParamsSource(nameof(RowCounts))]
    public int Rows { get; set; }




    [GlobalSetup(Target = nameof(Read))]
    public void GlobalSetupRead()
    {
        var file = BenchmarkFixtures.LoadReadFile("RealDoublesDictionary");
        _pinned = GCHandle.Alloc(file, GCHandleType.Pinned);
        _buffer = new NativeBuffer(_pinned.AddrOfPinnedObject(), file.LongLength);
        _columnSources = new BufferReader[10];
        for (var index = 0; index < 10; index++)
            _columnSources[index] = new BufferReader(_buffer);
        _source = _columnSources[0];
        _columnReaders = new ParquetFileReader[10];
        _columnCounts = new long[10];
        _columnWorkerCount = ColumnParallelism.WorkerCount(10);
    }
    [IterationSetup(Target = nameof(Read))]
    public void SetupRead()
    {
        foreach (var reader in _columnReaders ?? [])
            reader?.Dispose();
        _columnReaders = new ParquetFileReader[10];
        for (var index = 0; index < 10; index++)
            _columnReaders[index] = new ParquetFileReader(_columnSources[index]);
        _reader = _columnReaders[0];
    }
    public double VerifyRead()
    {
        double sum = 0;
        long count = 0;
        for (var g = 0; g < _reader.FileMetaData.NumRowGroups; g++)
        {
            using var group = _reader.RowGroup(g);
            using (var column = group.Column(0).LogicalReader<double?>())
            {
                while (column.HasNext)
                {
                    var length = column.ReadBatch(_read0);
                    sum = ColumnChecksum.Accumulate(_read0.AsSpan(0, length), sum);
                    count += length;
                }
            }
            using (var column = group.Column(1).LogicalReader<double?>())
            {
                while (column.HasNext)
                {
                    var length = column.ReadBatch(_read1);
                    sum = ColumnChecksum.Accumulate(_read1.AsSpan(0, length), sum);
                    count += length;
                }
            }
            using (var column = group.Column(2).LogicalReader<double?>())
            {
                while (column.HasNext)
                {
                    var length = column.ReadBatch(_read2);
                    sum = ColumnChecksum.Accumulate(_read2.AsSpan(0, length), sum);
                    count += length;
                }
            }
            using (var column = group.Column(3).LogicalReader<double?>())
            {
                while (column.HasNext)
                {
                    var length = column.ReadBatch(_read3);
                    sum = ColumnChecksum.Accumulate(_read3.AsSpan(0, length), sum);
                    count += length;
                }
            }
            using (var column = group.Column(4).LogicalReader<double?>())
            {
                while (column.HasNext)
                {
                    var length = column.ReadBatch(_read4);
                    sum = ColumnChecksum.Accumulate(_read4.AsSpan(0, length), sum);
                    count += length;
                }
            }
            using (var column = group.Column(5).LogicalReader<double?>())
            {
                while (column.HasNext)
                {
                    var length = column.ReadBatch(_read5);
                    sum = ColumnChecksum.Accumulate(_read5.AsSpan(0, length), sum);
                    count += length;
                }
            }
            using (var column = group.Column(6).LogicalReader<double?>())
            {
                while (column.HasNext)
                {
                    var length = column.ReadBatch(_read6);
                    sum = ColumnChecksum.Accumulate(_read6.AsSpan(0, length), sum);
                    count += length;
                }
            }
            using (var column = group.Column(7).LogicalReader<double?>())
            {
                while (column.HasNext)
                {
                    var length = column.ReadBatch(_read7);
                    sum = ColumnChecksum.Accumulate(_read7.AsSpan(0, length), sum);
                    count += length;
                }
            }
            using (var column = group.Column(8).LogicalReader<double?>())
            {
                while (column.HasNext)
                {
                    var length = column.ReadBatch(_read8);
                    sum = ColumnChecksum.Accumulate(_read8.AsSpan(0, length), sum);
                    count += length;
                }
            }
            using (var column = group.Column(9).LogicalReader<double?>())
            {
                while (column.HasNext)
                {
                    var length = column.ReadBatch(_read9);
                    sum = ColumnChecksum.Accumulate(_read9.AsSpan(0, length), sum);
                    count += length;
                }
            }
        }
        if (count != (long)Rows * 10) throw new InvalidDataException($"Expected {(long)Rows * 10} values, got {count}.");
        return sum;
    }

    long ReadColumn0()
    {
        long count = 0;
        for (var g = 0; g < _columnReaders[0].FileMetaData.NumRowGroups; g++)
        {
            using var group = _columnReaders[0].RowGroup(g);
            using var column = group.Column(0).LogicalReader<double?>();
            while (column.HasNext)
            {
                var length = column.ReadBatch(_read0);
                ReadConsumption.Consume(_read0.AsSpan(0, length));
                count += length;
            }
        }
        return count;
    }

    long ReadColumn1()
    {
        long count = 0;
        for (var g = 0; g < _columnReaders[1].FileMetaData.NumRowGroups; g++)
        {
            using var group = _columnReaders[1].RowGroup(g);
            using var column = group.Column(1).LogicalReader<double?>();
            while (column.HasNext)
            {
                var length = column.ReadBatch(_read1);
                ReadConsumption.Consume(_read1.AsSpan(0, length));
                count += length;
            }
        }
        return count;
    }

    long ReadColumn2()
    {
        long count = 0;
        for (var g = 0; g < _columnReaders[2].FileMetaData.NumRowGroups; g++)
        {
            using var group = _columnReaders[2].RowGroup(g);
            using var column = group.Column(2).LogicalReader<double?>();
            while (column.HasNext)
            {
                var length = column.ReadBatch(_read2);
                ReadConsumption.Consume(_read2.AsSpan(0, length));
                count += length;
            }
        }
        return count;
    }

    long ReadColumn3()
    {
        long count = 0;
        for (var g = 0; g < _columnReaders[3].FileMetaData.NumRowGroups; g++)
        {
            using var group = _columnReaders[3].RowGroup(g);
            using var column = group.Column(3).LogicalReader<double?>();
            while (column.HasNext)
            {
                var length = column.ReadBatch(_read3);
                ReadConsumption.Consume(_read3.AsSpan(0, length));
                count += length;
            }
        }
        return count;
    }

    long ReadColumn4()
    {
        long count = 0;
        for (var g = 0; g < _columnReaders[4].FileMetaData.NumRowGroups; g++)
        {
            using var group = _columnReaders[4].RowGroup(g);
            using var column = group.Column(4).LogicalReader<double?>();
            while (column.HasNext)
            {
                var length = column.ReadBatch(_read4);
                ReadConsumption.Consume(_read4.AsSpan(0, length));
                count += length;
            }
        }
        return count;
    }

    long ReadColumn5()
    {
        long count = 0;
        for (var g = 0; g < _columnReaders[5].FileMetaData.NumRowGroups; g++)
        {
            using var group = _columnReaders[5].RowGroup(g);
            using var column = group.Column(5).LogicalReader<double?>();
            while (column.HasNext)
            {
                var length = column.ReadBatch(_read5);
                ReadConsumption.Consume(_read5.AsSpan(0, length));
                count += length;
            }
        }
        return count;
    }

    long ReadColumn6()
    {
        long count = 0;
        for (var g = 0; g < _columnReaders[6].FileMetaData.NumRowGroups; g++)
        {
            using var group = _columnReaders[6].RowGroup(g);
            using var column = group.Column(6).LogicalReader<double?>();
            while (column.HasNext)
            {
                var length = column.ReadBatch(_read6);
                ReadConsumption.Consume(_read6.AsSpan(0, length));
                count += length;
            }
        }
        return count;
    }

    long ReadColumn7()
    {
        long count = 0;
        for (var g = 0; g < _columnReaders[7].FileMetaData.NumRowGroups; g++)
        {
            using var group = _columnReaders[7].RowGroup(g);
            using var column = group.Column(7).LogicalReader<double?>();
            while (column.HasNext)
            {
                var length = column.ReadBatch(_read7);
                ReadConsumption.Consume(_read7.AsSpan(0, length));
                count += length;
            }
        }
        return count;
    }

    long ReadColumn8()
    {
        long count = 0;
        for (var g = 0; g < _columnReaders[8].FileMetaData.NumRowGroups; g++)
        {
            using var group = _columnReaders[8].RowGroup(g);
            using var column = group.Column(8).LogicalReader<double?>();
            while (column.HasNext)
            {
                var length = column.ReadBatch(_read8);
                ReadConsumption.Consume(_read8.AsSpan(0, length));
                count += length;
            }
        }
        return count;
    }

    long ReadColumn9()
    {
        long count = 0;
        for (var g = 0; g < _columnReaders[9].FileMetaData.NumRowGroups; g++)
        {
            using var group = _columnReaders[9].RowGroup(g);
            using var column = group.Column(9).LogicalReader<double?>();
            while (column.HasNext)
            {
                var length = column.ReadBatch(_read9);
                ReadConsumption.Consume(_read9.AsSpan(0, length));
                count += length;
            }
        }
        return count;
    }

    long ReadColumn(int ordinal)
        => ordinal switch
        {
            0 => ReadColumn0(),
            1 => ReadColumn1(),
            2 => ReadColumn2(),
            3 => ReadColumn3(),
            4 => ReadColumn4(),
            5 => ReadColumn5(),
            6 => ReadColumn6(),
            7 => ReadColumn7(),
            8 => ReadColumn8(),
            9 => ReadColumn9(),
            _ => throw new ArgumentOutOfRangeException(nameof(ordinal))
        };

    [Benchmark]
    public long Read()
    {
        _parallelism.Run(10, _columnWorkerCount,
            index => _columnCounts[index] = ReadColumn(index));
        long count = 0;
        for (var index = 0; index < 10; index++)
            count += _columnCounts[index];
        if (count != (long)Rows * 10)
            throw new InvalidDataException($"Expected {(long)Rows * 10} values, got {count}.");
        _readThreads = _parallelism.LastObserved;
        return count;
    }

    [GlobalCleanup]
    public void Cleanup()
    {
        if (_readThreads > 0)
            ColumnParallelism.WriteMarker("RealDoublesDictionaryColumnMultiParquetSharpBenchmarks", "read", _columnWorkerCount, _readThreads);
        foreach (var reader in _columnReaders ?? [])
            reader?.Dispose();
        foreach (var source in _columnSources ?? [])
            source?.Dispose();
        _buffer?.Dispose();
        if (_pinned.IsAllocated) _pinned.Free();
    }
}
