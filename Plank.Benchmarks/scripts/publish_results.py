    ...
    return {
        ...
        "configuration": {
            **measurement_configuration(args.log.read_text()),
            ...
            "quick": False,
            "rowGroupBoundaries": "Row workloads receive flat rows; column workloads receive column batches with matching row counts per row group. Synthetic cases produce 22 row groups; taxi-derived cases produce 3. Plank ColumnMulti reads schedule every row-group/column pair through PLINQ at Environment.ProcessorCount; ParquetSharp ColumnMulti reads schedule independent columns.",
            "timingBoundary": "Only the benchmark write/read method is timed, not process startup. Each case runs its measured iterations in one process. With ColdStart and zero warmups, the first operation is not pre-invoked by setup or JIT calibration; later samples show its evolution. Read fixtures and write output capacities are prepared in separate processes once per run. Data, schemas, options, capacities, streams, and reusable writer/reader setup remain outside timing where the public API permits it. Plank ColumnMulti PLINQ scheduling and any worker startup occur inside the timed read. ColumnMulti cases record configured workerCount plus observedThreads in each measurement. " + isolation,
        },
        "suites": suites,
