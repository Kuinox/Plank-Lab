...
/*
 * Keep page sizes close to the requested row budget, but move each interior
 * boundary to a nearby change in delta direction.  This makes an isolated
 * timestamp reversal start a fresh DELTA stream without making the number of
 * pages data-dependent at the large scale.
 */
static uint64_t transition_boundary(const int64_t *values, uint64_t total,
                                    uint64_t first, uint64_t page_rows) {
    uint64_t nominal = first + page_rows;
    if (nominal > total) nominal = total;
    if (nominal == total || nominal <= first + 1) return nominal;
    /* This candidate deliberately uses five equal independent pages. */
    return nominal;
    const uint64_t radius = 65536;
    uint64_t low = nominal > radius ? nominal - radius : 1;
    if (low <= first + 1) low = first + 2;
    uint64_t high = nominal + radius;
    if (high >= total) high = total - 1;
    uint64_t best = nominal;
    uint64_t best_magnitude = 0;
    uint64_t best_distance = UINT64_MAX;
    for (uint64_t boundary = low; boundary <= high; ++boundary) {
        int before_negative = values[boundary - 1] < values[boundary - 2];
        int after_negative = values[boundary] < values[boundary - 1];
        /* Only an upward-to-downward transition places the disruptive
         * negative delta across the page boundary, so that delta becomes
         * the next page's omitted first delta. */
        if (before_negative || !after_negative) continue;
        uint64_t distance = boundary > nominal ? boundary - nominal : nominal - boundary;
        int64_t delta = values[boundary] - values[boundary - 1];
    ...
    if (best_magnitude) return best;
    /* A boundary without a nearby negative run remains at the row budget. */
    for (uint64_t boundary = low; boundary <= high; ++boundary) {
        int before_negative = values[boundary - 1] < values[boundary - 2];
        int after_negative = values[boundary] < values[boundary - 1];
...
int main(int argc, char **argv) {
    ...
    if (*(const uint16_t *)"\1\0" != 1) die("this starter requires a little-endian host");
    ...
    uint64_t page_count64 = 0;
    for (uint64_t first = 0; first < total_rows;) {
        uint64_t end = transition_boundary(values, total_rows, first, options.page_rows);
        if (end <= first) die("invalid transition page boundary");
        ++page_count64;
        first = end;
    }
    ...
    set_zstd_parameter(zstd, ZSTD_c_contentSizeFlag, 0, "contentSizeFlag");
    ...
    set_zstd_parameter(zstd, ZSTD_c_targetCBlockSize, 131072, "targetCBlockSize");
